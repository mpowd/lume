...
async def get_collection_info(collection_name: str):
    try:
        ...
        collection_info = mongodb_client.get_docs(filter={"collection_name": collection_name}, collection_name="configurations")
        
        if len(collection_info) > 1:
            logger.warning(f"Warning: In MongoDB collection 'configurations' found more than one config file for '{collection_name}', using the first one")
            return json.loads(json_util.dumps(collection_info[0]))
        elif len(collection_info) == 0:
            logger.error(f"Error: In mongo db collection 'configurations' could not be found a config file for the collection '{collection_name}'")
            raise HTTPException(
                status_code=404,
...
def get_embedder(embedding_model: str):
    match embedding_model:
        case 'jina/jina-embeddings-v2-base-de':
            return {
                "dense_embeddings": OllamaEmbeddings(model="jina/jina-embeddings-v2-base-de", base_url="http://ollama:11434"),
...
async def create_collection(request: CollectionCreateRequest):
    ...
    try:
        qdrant_client.create_collection(
            ...
            vectors_config={"dense": VectorParams(size=embedding_dim, distance=distance_metric)},
            sparse_vectors_config={
                "sparse": SparseVectorParams(index=models.SparseIndexParams(on_disk=False))
            },
        )
    except Exception as e:
        logger.error(f"Error creating qdrant collection {request.collection_name}: {str(e)}")
        raise HTTPException(
            status_code=500,
    ...
    try:
        ...
        logger.error(f"Error saving config document for qdrant collection '{request.collection_name}' in mongodb: {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def upload_documents(request: UploadDocumentsRequest):
    ...
    try:
        ...
        collection_info = mongodb_client.get_docs(filter={"collection_name": request.collection_name}, collection_name="configurations")
        if not collection_info:
            raise HTTPException(status_code=404, detail=f"Collection configuration not found for {request.collection_name}")
        ...
        for normalized_url in normalized_urls:
            existing_url_docs = mongodb_client.get_docs_by_url(source_url=normalized_url, collection_name=request.collection_name)
            ...
            if not existing_url_docs:
                ...
                for variant in variant_urls:
                    variant_docs = mongodb_client.get_docs_by_url(source_url=variant, collection_name=request.collection_name)
                    if variant_docs and isinstance(variant_docs, list) and len(variant_docs) > 0:
                        existing_url_docs = variant_docs
                        break
            
            if existing_url_docs and isinstance(existing_url_docs, list) and len(existing_url_docs) > 0:
                existing_docs.add(normalized_url)
                sorted_docs = sorted(existing_url_docs, key=lambda x: x.get('timestamp', ''), reverse=True)
                latest_doc = sorted_docs[0]
                doc_markdown = latest_doc.get('markdown', '')
                if doc_markdown:
                    url_to_docs_map[normalized_url] = doc_markdown
                valid_urls.append(normalized_url)
                logger.info(f"URL {normalized_url} already exists in MongoDB collection {request.collection_name}")
        ...
        for url in urls:
            ...
            docs = mongodb_client.get_docs_by_url(source_url=url, collection_name="temp")
            if docs and isinstance(docs, list) and len(docs) > 0:
                ...
                if not latest_doc.get('metadata'):
                    latest_doc['metadata'] = {}
                latest_doc['metadata']['content_hash'] = generate_content_hash(doc_markdown)
                
                latest_doc['metadata']['normalized_url'] = normalized_url
                
                latest_doc['source_url'] = normalized_url
                
                mongodb_client.persist_docs(docs=[latest_doc], collection_name=request.collection_name)
        ...
        logger.info(f"Found {len(existing_hashes)} existing document hashes in collection")
        ...
        if all_chunks:
            embedder_info = get_embedder(collection_info[0].get('dense_embedding_model'))
            if embedder_info == -1:
                raise HTTPException(status_code=400, detail=f"Unsupported embedding model")
...
async def check_url_exists(collection_name: str, url: str):
    ...
    try:
        ...
        docs = mongodb_client.get_docs_by_url(source_url=normalized_url, collection_name=collection_name)
        ...
        if not docs or len(docs) == 0:
            ...
            for variant in variant_urls:
                ...
                variant_docs = mongodb_client.get_docs_by_url(source_url=variant, collection_name=collection_name)
                if variant_docs and isinstance(variant_docs, list) and len(variant_docs) > 0:
                    docs = variant_docs
                    break
        ...
        raise HTTPException(status_code=500, detail=f"Error checking URL existence: {str(e)}")
...
async def deduplicate_collection(collection_name: str):
    ...
    try:
        ...
        duplicate_groups = {url: docs for url, docs in url_groups.items() if len(docs) > 1}
        ...
        duplicate_hashes = {hash: points for hash, points in hash_groups.items() if len(points) > 1}
        point_ids_to_remove = []
        ...
        raise HTTPException(status_code=500, detail=f"Error deduplicating collection: {str(e)}")
...
async def get_existing_content_hashes(collection_name: str) -> Set[str]:
    ...
    try:
        ...
        logger.error(f"Error retrieving content hashes from collection {collection_name}: {str(e)}")
        return set()
