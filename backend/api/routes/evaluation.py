...
class QAPair(BaseModel):
    question: str = Field(description="Eine realistische Frage eines Bürgers zum Dokument")
    answer: str = Field(description="Eine faktenbasierte Antwort auf die Frage aus dem Dokument")
...
async def create_dataset(request: DatasetCreationRequest):
    ...
    try:
        ...
        if mongodb_client.get_collection(collection_name="evaluation_datasets").find_one({"name": request.dataset_name}):
            return {
                "status": "error",
                "message": f"A dataset with the name '{request.dataset_name}' already exists. Please choose another name."
            }

        inserted_id = mongodb_client.persist_docs(docs=[dataset_entry], collection_name="evaluation_datasets")
        
        dataset_entry["_id"] = str(inserted_id[0]) if isinstance(inserted_id, list) else str(inserted_id)
...
async def get_evaluation_datasets():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving evaluation datasets: {str(e)}")
...
async def update_evaluation_dataset(dataset_id: str, update_data: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating evaluation dataset: {str(e)}")
...
async def generate_ragas_dataset(request: RagasDatasetGenerationRequest):
    try:
        ...
        logger.info(f"First row sample: {ragas_data.iloc[0].to_dict() if not ragas_data.empty else 'Empty'}")
        ...
        raise HTTPException(status_code=500, detail=f"Error generating RAGAS dataset: {str(e)}")
...
class EvaluationRequest(BaseModel):
    ...
    ground_truths: List[str] = Field(description="Ground truth answers for the questions")
    answers: List[str] = Field(description="Answers from the RAG system for the questions")
    retrieved_contexts: List[str] = Field(description="Retrieved context chunks for the questions from the RAG system")
...
class EvaluationRequest(BaseModel):
    ...
    ground_truths: List[str] = Field(description="Ground truth answers for the questions")
    answers: List[str] = Field(description="Answers from the RAG system for the questions")
    retrieved_contexts: List[List[str]] = Field(description="Retrieved context chunks for the questions from the RAG system, each question has a list of context chunks")
...
async def evaluate(request: EvaluationRequest):
    ...
    try:
        ...
        if len(df) > 0:
            ...
            logger.info(f"Sample row retrieved_contexts type: {type(sample_row['retrieved_contexts'])}")
            if isinstance(sample_row['retrieved_contexts'], list):
                logger.info(f"Sample row retrieved_contexts length: {len(sample_row['retrieved_contexts'])}")
                if len(sample_row['retrieved_contexts']) > 0:
                    logger.info(f"First context type: {type(sample_row['retrieved_contexts'][0])}")
        ...
        evaluation_results["_id"] = str(inserted_id[0]) if isinstance(inserted_id, list) else str(inserted_id)
...
async def evaluate_chatbot(request: EnhancedEvaluationRequest):
    ...
    try:
        ...
        chatbot_info = mongodb_client.get_docs(
            filter={"_id": ObjectId(request.chatbot_id)},
        ...
        dataset_info = mongodb_client.get_collection("evaluation_datasets").find_one({"name": request.dataset_name})
        ...
        evaluation_results = {
            ...
            "dataset": {
                ...
                "source_collection": dataset_info.get("source_collection") if dataset_info else None,
                "generated_at": dataset_info.get("generated_at") if dataset_info else None,
        ...
        import traceback
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Error evaluating chatbot: {str(e)}")
...
async def get_evaluation_by_id(evaluation_id: str):
    ...
    try:
        ...
        mongodb_client = MongoDBClient.get_instance()
        evaluation = mongodb_client.get_collection("evaluations").find_one({"_id": ObjectId(evaluation_id)})
