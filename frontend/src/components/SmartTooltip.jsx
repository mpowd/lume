...
const SmartTooltip = ({ source, context, index, getSourceUrl, getSourceScore, getSourceDomain, getSourceStyle, getDotColor, collectionName }) => {
  ...
  const getFileLinkUrl = () => {
    if (!isFile) return url

    const match = url.match(/^data\/files\/(.+)\/([^/]+)$/)
    if (!match) {
      console.error('Cannot build file URL: unexpected path format', { url, source })
      return null
    }

    const [, collectionName, filename] = match
    return `/knowledge-base/files/${encodeURIComponent(collectionName)}/${encodeURIComponent(filename)}`
  }

  const linkUrl = getFileLinkUrl()
