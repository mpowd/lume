...
export default function AdvancedSettings({ 
  ...
  const isKnowledgeSourceSelected = formData.collections.length > 0;
    ...
    >
      <div className="space-y-6 p-6 bg-transparent rounded-xl border border-white/5">
        {isKnowledgeSourceSelected ? (
          ...
          // Display only the editable prompt when no knowledge source is selected
          <div className="space-y-4">
            <FormTextarea
              label={formData.precise_citation ? "Precise Citation Prompt" : "RAG Prompt Template"}
              value={formData.precise_citation ? formData.precise_citation_prompt : formData.rag_prompt}
              onChange={(e) => setFormData({
                ...formData,
                [formData.precise_citation ? 'precise_citation_prompt' : 'rag_prompt']: e.target.value
              })}
              rows={6}
              className="font-mono text-sm"
            />
            ...
            <button
              type="button"
              onClick={() => {
                if (formData.precise_citation) {
                  setFormData({...formData, precise_citation_prompt: defaultPreciseCitationPrompt});
                } else {
                  setFormData({...formData, rag_prompt: defaultRagPrompt});
                }
              }}
              className="text-xs text-text-tertiary hover:text-white transition-colors"
            >
              {formData.precise_citation ? "Reset to default precise citation prompt" : "Reset to default RAG prompt"}
            </button>
          </div>
        )}
      </div>
